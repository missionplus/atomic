...
> - `reference/discovery.md` — how to find integration surfaces
> - `reference/fidelity-rubric.md` — when to use a canned stub vs a simple transform
> - `reference/fragility-rubric.md` — how to flag risky surfaces
> - `reference/manifest-schema.md` — the `fake.manifest.yaml` format
> - `reference/templates/` — starter files for Python and Node fakes
...
Open `reference/discovery.md` and follow it. The short version:
...
Open `reference/fidelity-rubric.md`. For each surface, mark it as one of:
...
Use the templates in `reference/templates/` as starting points. Adapt — do not copy verbatim; the names, routes, and schemas should come from the actual app.

### 6. Write the manifest

The `fake.manifest.yaml` is critical — `update-fake` depends on it. Follow `reference/manifest-schema.md` exactly. Record:
...
Inside `/fake/FRAGILITY.md`, list surfaces or patterns flagged using `reference/fragility-rubric.md`. For each, write:
...
Inside `/fake/consumer-config.example.env`, list env vars a consuming app sets to point at the fake instead of the real dependency. Use the patterns in `reference/templates/consumer-config.example.env`.
